...
description: |
  Learn how to use R for International Merchandise Trade Statistics. Focus is on tidyverse tools: select, filter, group_by, summarise, pivot_wider, and ggplot2 for visualisation. Data from UN Comtrade.
resource_files:
- www/SITCCodeandDescription_light.xlsx
- www/IMTS_Correspondence_light.xlsx
---
...
# tradedata_raw <- read.csv("./RawData/tradedata_ready.csv")
# trade_Asia <- tradedata_raw %>%
#   select(-c("X", "motCode")) %>%
#   filter(regional.grouping =="Asia") %>%
#     filter( geoAreaDescription %in% c("China", "Mongolia", "Indonesia",  "Japan", "Singapore" ,
#                                       "India", "Philippines", "Guam", "Australia" , "New Zealand", "Thailand" ))
# 
# write.csv(trade_Asia, "www/tradeAsiaNew.csv",row.names = FALSE)

tradedata <- read.csv("./www/tradeAsiaNew.csv")
tradedata <- tradedata %>%
...
# From Bimlesh for new section with SITC classification

# HS_SITC_concordance     <- read_excel("./www/IMTSCorrespondence_light.xlsx", sheet = "HS SITC BEC") %>%
#   rename(HS_Code = `HS22`, SITC_Code = SITC4) %>%
#   mutate(HS_Code = as.character(HS_Code), SITC_Code = as.character(SITC_Code))
# 
# SITC_description        <- read_excel("./www/SITCCodeandDescription_light.xlsx", sheet = "SITC4") %>%
#   rename(SITC_Code = Code, SITC_Description = Description) %>%
#   mutate(SITC_Code = as.character(SITC_Code))
...
```{r tradeset, exercise = TRUE, exercise.eval = FALSE}
#  run the code below
head(tradedata)
...
```{r tradeset-hint-1}
# Click on the top right button "> Run Code"  
head(tradedata)
```



The dataset has **`r nrow(tradedata)`** observations and **`r ncol(tradedata)`** variables. As this is an extract from a real data set, it may happen that some variables have missing values for some observations and some variables (as *FOBValue*)
...
quiz(
  ...
  question("What values can `flowCode2` take?",
    ...
    answer('"Export" and "Import"', correct = TRUE,
           message = "Yes. `flowCode2` records the trade direction: `Export` or `Import`"),
...
The `select()` function lets you keep only the **variables** (columns) you want.
...
tradedata_light <- tradedata %>%
  select(flowCode2, geoAreaDescription,  ____ , ____,    )
```
...
We often want to filter some observations, based on some conditions. The `filter()` function keeps only the **rows** (observations) that satisfy a condition.
...
> **Hint**: Recall that exports are identified by `flowCode2 == "Export"`.
...
>**Hint**: You should use an additionnal condition involving the variable  `primaryvalue`.
...
large_exports <- tradedata %>%
  filter(flowCode2 == "Export" & primaryValue > ___)
```
...
quiz(
  ...
  question("Does the order of filter() and select() matter in a pipeline?",
    answer("No, the result is always the same", correct = FALSE, 
           message = " If you select() first and drop a column you later need in filter(), the pipeline will fail."),
...
table1 <- tradedata %>%
  group_by(flowCode2) %>%
  summarise(total_trade_value = sum( ______ / 1000000) )
```

```{r grp1-solution}
# Think that you may have missing values (NA)
table1 <- tradedata %>%
  group_by(flowCode2) %>%
  summarise(total_trade_value = sum(primaryValue / 1000000, na.rm = TRUE),
            .groups = "drop")
...
> Name the value column `Value` and remane the country column `Area`.
...
table2_long <- tradedata %>%
  group_by(____ , ______) %>%
  summarise( Value = sum(_____ / _______ , na.rm = TRUE)  )
...
> **Your goal:** starting from `table2_long` (created above), create `table2_wide` in wide format:  
...
**This is `table2_wide`** — the main partner-country summary table. You can see at a glance which countries are the largest export destinations.
...
- Start by grouping the data by product (`cmd2CodeDescription`)  and trade flow (`flowCode2` ), then compute the total trade value for each group.
- Reshape the result so flows become columns (use `pivot_wider()`)


```{r prod-setup, include=FALSE}
# tradedata is available
```
...
> **What are the country's top export products?**


## Step 8 — Visualise with `ggplot2`

Tables are informative, but a chart communicates patterns much faster. 

>  **Your goal**: Build a *horizontal bar chart* showing exports and imports for the **top 5 partner countries**.

**Hints:**

- First, in `table2`,  keep only the top 5 countries by export value (use `slice()` after a wise ordering)
- Then, convert `table2`  to **long format** for `ggplot2` (use `pivot_longer()`) 
- Finally,  plot the result using `ggplot()` with a  `geom_col()` for a bar chart with counts and `coord_flip()` to flip the graphic horizontally

**Wait!** We've done that for you below — your task is just to run the code and see the output ;-)

```{r chart-setup, include=FALSE}
...
# Reshape top 5 to long format
table2_long_top5 <- table2_wide %>%
  slice_max(order_by = Export, n = 5) %>%
...
# table2_long_top5 is ready to use ;-)
# Run the call below:

ggplot(table2_long_top5,
  ...
  coord_flip() +
  labs(
    title = "Trade flows by top 5 partner countries (million USD)",
    x = "Partner country",
    y = "Value (million USD)",
    fill = "Trade flow"
  ) +
...
```{r chart1-solution}
# This is a copy of the code prepared to be executed- just start over and execute!
...
> **Now try it for products:** can you adapt this code to plot the top 5 products from `table3`?

**Hints:**

- Start by keeping the 5 largest exports from table3
- Reshape the data frame  table3 to long format (created `table3_long`) for ggplot (use `pivot_longer()`) 
- Adapt the previous ggplot code to products using `table3_long`


```{r chart2-setup, include=FALSE}
# table3 is pre-computed and available here
table3 <- tradedata %>%
  group_by(cmd2CodeDescription, flowCode2) %>%
  summarise(total_trade_value = sum(primaryValue / 1000000, na.rm = TRUE),
            .groups = "drop") %>%
...
```{r chart2, exercise = TRUE, exercise.eval = FALSE, exercise.setup = "chart2-setup", exercise.cap = "Your turn: bar chart by product"}
# Adapt the chart above to show the top 5 product categories
...
ggplot(table3_long,
  ...
  geom_col(position = position_dodge(width = 0.9)) +
  coord_flip() +
  labs(title = "Trade flows by top 5 product categories (million USD)",
...
ggplot(table3_long,
  ...
  geom_col(position = position_dodge(width = 0.9)) +
  coord_flip() +
  labs(title = "Trade flows by top 5 exported commodities  (million USD)",
       x = "Product", y = "Value (million USD)", fill = "Trade flow") +
  scale_x_discrete(labels = function(x) str_wrap(str_trunc(x, 40), width = 30)) +
  theme_minimal()
```

## Extra step: Trade by other classifications (SITC)

Trade data usually starts out coded in the **Harmonized System (HS)**, but other classifications — like **SITC** (Standard International Trade Classification) — are often used for economic analysis, since they group products by degree of processing rather than by physical/technical characteristics.

> **Your goal:** create `table4` — total export and import values by **SITC section**, in **wide format**, using the HS-to-SITC concordance table `HS_SITC_concordance` to convert `cmdCode` into `SITC_Code`.

**Hints:**

- First add a new column with the SITC code, using `lookup()` to match `cmdCode` against the concordance column `HS17` table `HS_SITC_concordance`, and retrieve the `SITC_Code` from that table. 
- Then group by the SITC classification and trade flow (`flowCode2`), and compute total trade value for each group.
- Reshape the result so flows become columns (use `pivot_wider()`).

**Note:**
`lookup()` grabs a single matching value for each row, like Excel's *VLOOKUP*. Previously, we used tidyverse join (e.g. left_join()) which matches whole rows between tables and brings across all their columns at once. `lookup()` is an alternative, closer to Excel. 

```{r classif-setup, include=FALSE}
# tradedata and HS_SITC_concordance are available


```

```{r classif1, exercise = TRUE, exercise.eval = FALSE, exercise.setup = "classif-setup", exercise.cap = "Trade by SITC classification (wide format)"}
# Enter your solution below
table4 <-
```

```{r classif1-hint-1}
# Start by adding the SITC code with lookup(), then group_by and summarise
table4 <- tradedata %>%
  mutate(SITC_Code = lookup(cmdCode, HS_SITC_concordance$HS17, HS_SITC_concordance$SITC_Code)) %>%
  group_by(SITC_Code, flowCode2) %>%
  summarise(total_trade_value = sum(primaryValue / 1000000, na.rm = TRUE),
            .groups = "drop")

# This table will have 2 rows (import, exports)  per SITC code and needs to be reshaped  
head(table4, 5)

```

```{r classif1-hint-2}
# The table4 has two 2 rows (import, exports)  per SITC code and needs to be reshaped  
# Use pivot_wider to transform to wide format, filling missing combinations with 0
table4 <- tradedata %>%
  mutate(SITC_Code = lookup(cmdCode, HS_SITC_concordance$HS17, HS_SITC_concordance$SITC_Code)) %>%
  group_by(SITC_Code, flowCode2) %>%
  summarise(total_trade_value = sum(primaryValue / 1000000, na.rm = TRUE),
...
```{r classif1-solution}
# Sorting the table by SITC code
table4 <- tradedata %>%
  mutate(SITC_Code = lookup(cmdCode, HS_SITC_concordance$HS17, HS_SITC_concordance$SITC_Code)) %>%
  group_by(SITC_Code, flowCode2) %>%
  summarise(total_trade_value = sum(primaryValue / 1000000, na.rm = TRUE),
...
- `lookup()` matches each `cmdCode` against the concordance table's `HS17` column and returns the corresponding `SITC_Code` — this is how you move data from one classification system to another.
- Grouping by `SITC_Code` instead of `cmd2CodeDescription` gives the same trade totals, just organized by a different logic (economic function rather than product type).
- As in Step 6, `values_fill = 0` avoids `NA`s for SITC sections with no recorded exports or imports.


> **Now:** your goal is to create `table5` — total export and import values by **SITC section**, with description of the categories, using the  table `SITC_description` to include  `SITC_description` together with the `SITC_Code`.

**Hints:**

- Use Table 4 as input
- Add a new column with the SITC code **description**, using `lookup()` to match `SITC_Code` against the column `SITC_Description`  in the  SITC description table `SITC_description`.
- Reorder variables and sort by SITC code. 



```{r classif2-setup, include=FALSE}
# table4, HS_SITC_concordance and SITC_description  need to be available
...
```{r classif2, exercise = TRUE, exercise.eval = FALSE, exercise.setup = "classif2-setup", exercise.cap = "Trade by SITC classification (wide format)"}
# Enter your solution below
table5 <-
```

```{r classif2-hint-1}
# Start by adding the SITC code description with lookup(), 
table5 <- table4 %>%
  mutate(SITC_description = lookup(SITC_Code, SITC_description$SITC_Code,  
                                   SITC_description$SITC_Description))
```


```{r classif2-solution}
# Select the columns needed and sort according to SITC code
table5 <- table4 %>%
  mutate(SITC_description = lookup(SITC_Code, SITC_description$SITC_Code,
                                   SITC_description$SITC_Description)) %>% 
  select(SITC_description, SITC_Code, Export, Import ) %>% 
  arrange(SITC_Code)
...
> **Does the picture change when you look at trade through SITC instead of HS product categories?**
...
* **Continue learning** through platforms like [R for Data Science](https://r4ds.had.co.nz/), [Posit recipes](https://posit.cloud/learn/recipes), and  [A Modern Dive into R and the Tidyverse](https://moderndive.com/) among many useful references. You can also use *ChatGPT* or  *Claude* to help you with some simple requests and code debugging. Be careful though of mistakes, fake packages and confidential information! 
