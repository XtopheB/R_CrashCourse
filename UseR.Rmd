...
# Create an interactive learning quiz
quiz(
  "Rural Household Income Analysis",
  list(
    question("What is the average income of rural households?"),
    code({
      household_rural <- household_survey %>%
        filter(location == "Rural")

      # Calculate average income using the mean function
      avg_income <- mean(household_rural$income, na.rm = TRUE)
      avg_income
    }),
    answer(12500),
    hint("First, filter the data to include only rural households. Then, calculate the average income using the mean function.")
  ),
  list(
    question("How would you handle missing income data when calculating the average?"),
    code({
      # User's code goes here
    }),
    answer(contains("na.rm = TRUE") | contains("is.na")),
    hint("Use the `na.rm` argument in the `mean()` function or check for missing values with `is.na()` before calculating the average.")
  ),
  list(
    question("What is the average income of rural households using a different approach?"),
    code({
      # User's code goes here
    }),
    answer(contains("group_by") & contains("summarise")),
    hint("Try using `group_by()` and `summarise()` to calculate the average income.")
  )
)
```
...
# Step 4: Computing Average Income
```{r}
avg_income <- household_survey %>%
  filter(location == "Rural") %>%
  summarize(avg_income = sum(income) / n())
...
quiz(
  ...
  question("Which function helps to compute the total number of households?",
