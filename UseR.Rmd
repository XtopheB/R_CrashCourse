...
```{r quiz3, echo = FALSE}
quiz(
  question("In the *envipe2019*, are all variables encoded as **factors**? (look above,  after *@ format.spss*)",
...
Let's work here with: $$ A = \begin{bmatrix}
1 & 2  \\
3 & 4  
\end{bmatrix}  \;  and \;  B = 
\begin{bmatrix}
...
***Division*** is not a standard matrix operation. This is often approached by multiplying the lead matrix with an inverse of the lag matrix. The column dimension of the lead matrix should be the same as the row dimension of the lag matrix.  
...
incorrect = "Remember, `%*%` is used for matrix multiplication.",
allow_retry = TRUE
),
...
question("What value is returned by A[2,1]?",
...
You’ve learned how to produce a few lines of R code, import data and compute averages from survey data, how to compute simple indicators using different methods using a consistent and readable grammar of data science. You also have explored the simplicity and power of computation using **tidyverse** and even done a bit of of **matrix manipulation** that are often essential in statistics and SDG monitoring.
...
In this course, we focused on the **tidyverse** approach to R,  not because base R isn’t powerful, but because **tidyverse** offers a clearer, more consistent, and beginner-friendly grammar for working with data. This modern toolkit helps you think in terms of data pipelines, making your code easier to read, share, and scale across projects. 
