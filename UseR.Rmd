...
## Computing SDG 16.1.3

We now will earn how to compute SDG 16.1.3 - Proportion of persons subjected to physical, psychological or sexual harassment, in the previous12 months with disaggregation by sex.
...
```{r envipe1,  exercise = TRUE, exercise.eval = FALSE}
# Enter your solution below
...
### The indicator is a ratio of weighted sums 
...
$$
Indicator  =  \frac{Weighted\_Nb\_Victims}{Weighted\_Nb\_Population} \times 100 =  \frac{\sum_i 1_{[Victim_i==1]} \times weight_i }{\sum_i weight_i} \times 100
$$
...
- `mutate(TotalPop = sum(Weight))` calculates the row-sum of the weighted population for the full data set. The result is stored (as a constant value)  in the variable *WeightedPop*

- After filtering (*i.e.*, selecting only the victims: `AP7_3_11 == 1`), we compute the percentage in summarize(). sum(Weight) is done here only on the sub population of victims ( *i.e.*, our *numerator*) divided by the complete weighted population  generated by `unique(WeightedPop)` (our *denominator*). We have to use   `unique(WeightedPop)`  because *WeightedPop* is repeated across all rows, and we only want a single value for the denominator.^[We could have used `max(WeightedPop)` or `min(WeightedPop)` or even `mean(WeightedPop)`, they all provide the same value as `unique(WeightedPop)`.]


### Disagregating the SDG indicator 16.1.3 by Sex

The magic with the code we used for computing the indicator 16.1.3 is that it is extremely easy and straightforwad to compute that indicator for each value of the factor variable *SEXO*. 
...
SDG_16.3.3_Sex <- envipe2019_clean  %>%
  group_by(SEXO) %>%
  mutate(WeightedPop = sum(Weight)) %>%
  filter(AP7_3_11 ==1) %>%
  summarize( Indicator = 100* sum(Weight) / unique(WeightedPop)) 
