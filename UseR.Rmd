...
The core of the analysis is the transaction matrix , which shows how much each industry buys from and sells to the others.
...
rownames(Z) <- colnames(Z) <- c("IND1", "IND2", "IND3")
Z
```
...
The total output $X$ of each industry is the sum of intermediate consumption, imports, value-added, and final demand. To compute the sum of the intermediate consumption, we need to sum all column of the matrix $Z$. For that, we can use the function `colSums()`
. 
```{r}
# creating a vector with the sum of each column
colSums(Z)
```

The total output for each industry, is then the sum of the vectors representing the intermediate consumption (from Z), imports (M) , value-added (VA), and final demand (Y).

```{r}
# The total output is the sum  of 3 vectors
X <- colSums(Z) + M + VA + Y
X
```
