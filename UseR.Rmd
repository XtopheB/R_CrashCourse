...
```{r Popex, exercise.eval = TRUE}
household_rural <- household_survey %>%
  filter(location == "Rural")

```

```{r Popex1,  exercise = TRUE, exercise.eval = FALSE, exercise.setup = "Popex"}
...
```{r Popex1-hint-2}
# Now, you can filter!  Write the condition in the filter() function
household_rural <- household_survey %>%
  filter()
...
```{r Poex-Valid, exercise = TRUE, exercise.eval = FALSE, exercise.setup = "Popex"}
