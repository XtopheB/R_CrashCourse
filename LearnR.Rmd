...
### How to install R
The best way is to follow the instructions in this video (*3 min 55s*).

*You can download R from the <a href= "https://cran.r-project.org/" target="_blank">CRAN website </a>*

<video width="600" controls>
  <source src="https://unsiap.or.jp/on_line/2025/RCR/M0-RInstall.mp4" type="video/mp4">
...
<video width="600" controls>
  <source src="https://www.unsiap.or.jp/on_line/2025/RCR/M0-Packages.mp4" type="video/mp4">
  Your browser does not support the video tag.
</video>
...
R uses ` <- ` as the *assignment operator*. This is an arrow, formed as a combination of the *less than* symbol (**<**) and *hyphen* (**-**). Essentially, the assignment operator tells R to assign values or data on the **right side**  to a named object on the **left side**.
...
In the block of code above, we performed a simple mathematical operation **3 + 4** and stored the sum in an *object*  called **a**. Objects are used to store values for later use. The code above allows R to evaluate the expression 3 + 4 (on the right of the assignment operator) and then assign the sum 7, to the object a (on the left of the assignment operator), for later use.

> Notice that the result is not immediately visible in the console. Instead, you can see the name of the object in the R workspace, under the environment tab. To print out or view the content of the object **a** in the console, simply type the name of the object and run the command. You should now see the value (7) displayed in the console.

### Adding comments to your code:
It is a good practice to annotate your code in the R script editor. This includes adding short explanations on what the code or commands that you have written do as well as the meaning of any objects that are created during an R Session. These explanations are written as comments.
...
```{r, eval =TRUE}
# Create object a that stores the sum of 3 and 4
a <- 3 + 4
a
```
...
```{r}
# Here I define a numerical object 
myage <- 25
...
The **structure** of the data frame is given by `str()` function. It is very important to identify which column or variable is numerical (*num*) or an integer (*int*), a factor (*factor*) or even a string (*chr*, for character) or a Boolean (*logi* with values TRUE/FALSE) as some operations will only apply for some *type* of variables.
...
We can also have quick statistics using the  `summary()` function. As you see in the output, the `summary()`  function adapts the result to the **type** of the variable, whether it is a character (no summary for  *name*), a numeric variable (*age*), or a Boolean or logical variable (*student* takes TRUE or FALSE values).  

> This is an important feature of R functions and highlights the importance having **the correct *type*** for each variable in a data frame.  
...
![](https://uedufy.com/images/blog/en/how-to-import-a-csv-file-in-r-0.png){width=300px}

Then select the format of your file using the drop down menu. 

![](https://uedufy.com/images/blog/en/how-to-import-a-csv-file-in-r-1.png){width=300px}
