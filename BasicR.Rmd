...
$\hookrightarrow$ You **cannot** write `filter(SDG, region_name == "Asia" | "Africa" )`, even though you want all countries that are either in Asia or Africa.  You have to repeat the condition in full:

`filter(SDG, region_name == "Asia" | region_name == "Africa" )`

> This is one of the common mistake we all do! 

A useful solution for this problem is to use the ` %in%` operator.  In the condition:  `x %in% y`, we will select every row where *x* is one of the values **in** *y*. We could use it to rewrite the code above as:
