...
In the block of code above, we performed a simple mathematical operation **3 + 4** and stored the result (here the sum) in an object called ***a***. Objects are used to store values for later use. The code above allows R to evaluate the expression 3 + 4 (on the right of the assignment operator) and then assign the sum -  here 7 -  to the object ***a*** (on the left of the assignment operator), for later use.

> The assignment done in the first line is "silent" - nothing will be displayed. One must request the value of the object to be displayed, hence the second line.  
...
```{r, eval =TRUE}
# Create object, named a, that stores the sum of 3 and 4
a <- 3 + 4
a
```
...
The first line does the assignment to the object ** myage** (silently).  Now if I want to see the value, and  make it pretty, I can use the `paste()` function.  This function is quite useful for presenting results, titles, etc.,  as it *paste* text ( within quotes "  ") and other R objects (here a number). 
...
The **structure** of the data frame is given by `str()` function. It is very important to identify which *variable* (column) is numerical (*num*) or an integer (*int*), a factor (*factor*) or even a string (*chr*, for character) or a Boolean (*logi* with values TRUE/FALSE) as some operations will only apply for some *type* of variables.
