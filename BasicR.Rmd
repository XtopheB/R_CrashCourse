...
question("Check all that is TRUE:",
  ...
  type = "multiple",
  incorrect = "Note that will compute everything online here"
)
...
But, we can also extract eaxh cell of  the data frame `mydata` using its rows and columns numbers.  
...
> Remember that you only need to intall a package once, but have to load it at in every R session that uses it. 
...
> First, let's examine the `pipe` operator that pass the result of one expression into the next:  `%>%`. This is how we tell a story with code and, for example, move the data set from one transformation to another.^[In R, both `%>%`  (from tidyverse) and `|>`  (base R) can be used to pass the result of one expression into the next. We will work with the tidyverse where `%>%` has more features, but the logic is the same.]  
 
Imagine the following data story:  we are interested in only the first 3 variables and want to count the number of countries that are in the "Asia" region. This is how the code would translate that story: 
...
This requires quite a few lines of code, and a sequence of actions...

>  But there is a more readable solution


Then `%>% ` links operations in a very easy-to-read sequence. The code below does exactly the same operation as before, but passing the result of one expression into the next using  `%>% `.
...
Can you solve it and **count**  the number of countries who fulfill the conditions? (*see the `hints` to find out*).
...
The `summarise()` function is used to collapse multiple rows into a summary statistic, such as a total, average, min, max, etc. It's often used to calculate summaries by group.^[Using the operator `group_by()`for example. Although very convenient, this is not covered here.]
...
***What is the maximum maternal mortality ratio in the dataset?***
