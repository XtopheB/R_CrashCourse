...
- First,`table2` keep only the top 10 countries by export value (use `slice()` after a wise ordering)
- Then, convert `table2`  to **long format** for `ggplot2` (use `pivot_longer()`) 
- Finally,  plot the result using `ggplot()` with a  `geom_col()` for a bar chart with counts 
