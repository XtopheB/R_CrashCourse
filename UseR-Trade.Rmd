...
```{r}
# ---- New section on alternative STIC classifications

```
## Bonus — Trade by other classifications (SITC)
...
> **Your goal:** create `table4` — total export and import values by **SITC section**, in **wide format**, using the HS-to-SITC concordance table to convert `cmdCode` into `SITC_Code`.

**Hints:**

- First add a new column with the SITC code, using `lookup()` to match `cmdCode` against the concordance table `HS_SITC_concordance`.
- Then group by the SITC classification and trade flow (`flowCode2`), and compute total trade value for each group.
- Reshape the result so flows become columns (use `pivot_wider()`).

**Note**
`lookup()` grabs a single matching value for each row, like Excel's **VLOOKUP**. Previously, we used tidyverse join (e.g. left_join()) wich matches whole rows between tables and brings across all their columns at once. `lookup()` is an alternative, closer to Excel. 
...
```{r classif1-hint-2}
# Then pivot to wide format, filling missing combinations with 0
table4 <- tradedata %>%
  mutate(SITC_Code = lookup(cmdCode, HS_SITC_concordance$HS17, HS_SITC_concordance$SITC_Code)) %>%
  group_by(SITC_Code, flowCode2) %>%
  summarise(total_trade_value = sum(primaryValue / 1000000, na.rm = TRUE),
...
## Step 7bis — Trade by other classifications (SITC)

Trade data usually starts out coded in the **Harmonized System (HS)**, but other classifications — like **SITC** (Standard International Trade Classification) — are often used for economic analysis, since they group products by degree of processing rather than by physical/technical characteristics.

> **Your goal:** create `table_15` — total import values by `SITC_Code`, using the HS-to-SITC concordance table to convert `HS_Code6` into `SITC_Code`.

**Hints:**

- Add a new column with the SITC code, using `lookup()` to match `HS_Code6` against `HS_SITC_concordance`.
- Filter to keep only `Trade_Type == "Imports"`, then group by `SITC_Code` and sum the value column.

```{r sitc-setup, include=FALSE}
# tradedata and HS_SITC_concordance are available
```
```{r sitc1, exercise = TRUE, exercise.eval = FALSE, exercise.setup = "sitc-setup", exercise.cap = "Total imports by SITC code"}
# Enter your solution below
table_15 <-
```

```{r sitc1-hint-1}
# Start by adding SITC_Code with lookup(), then filter to Imports
table_15 <- tradedata %>%
  mutate(SITC_Code = lookup(HS_Code6, HS_SITC_concordance$HS17, HS_SITC_concordance$SITC_Code)) %>%
  filter(Trade_Type == "Imports")
```

```{r sitc1-hint-2}
# Then group_by SITC_Code and summarize the total import value
table_15 <- tradedata %>%
  mutate(SITC_Code = lookup(HS_Code6, HS_SITC_concordance$HS17, HS_SITC_concordance$SITC_Code)) %>%
  filter(Trade_Type == "Imports") %>%
  group_by(SITC_Code) %>%
  summarize(Total_Imports = sum(.data[[value_col]], na.rm = TRUE)) %>%
  arrange(SITC_Code)
```
...
- `lookup()` matches each `HS_Code6` against the concordance table's `HS17` column and returns the corresponding `SITC_Code`.
- `.data[[value_col]]` lets you reference a column by name stored in a variable, rather than typing the column name directly.

> **How many SITC categories do you end up with, compared to the number of HS codes you started with?**


###  Adding descriptions and a total row

Raw SITC codes are hard to interpret on their own — adding a text description, and a grand total row, makes the table ready to present.

> **Your goal:** extend `table_15` from Step 7 by adding `SITC_description`, keeping only the relevant columns, and appending a total row at the bottom.

**Hints:**

- Use `lookup()` again, matching `SITC_Code` against `SITC_description` to bring in the text label, then `select()` the three columns you need.

- Use `bind_rows()` with `summarise_all()` to add a total row at the end.

```{r sitc2-setup, include=FALSE}
# table_15 from Step 7, and SITC_description, are available
```


```{r sitc2, exercise = TRUE, exercise.eval = FALSE, exercise.setup = "sitc2-setup", exercise.cap = "Add SITC descriptions and a total row"}
# Enter your solution below
table_15 <- table_15 %>%
```
...
```{r sitc2-hint-1}
# Add the description column with lookup(), then select the columns you need
table_15 <- table_15 %>%
  mutate(SITC_description = lookup(SITC_Code, SITC_description$SITC_Code, SITC_description$SITC_Description)) %>%
  select(SITC_Code, SITC_description, Total_Imports)
```

```{r sitc2-hint-2}
# Then bind a total row using summarise_all()
table_15 <- table_15 %>%
  mutate(SITC_description = lookup(SITC_Code, SITC_description$SITC_Code, SITC_description$SITC_Description)) %>%
  select(SITC_Code, SITC_description, Total_Imports) %>%
  bind_rows(summarise_all(., ~if(is.numeric(.)) sum(.) else "Total Re-exports"))
```
...
- `summarise_all()` applies a function to every column of the table *as it exists at that point in the pipe*: numeric columns get summed (the grand total), non-numeric columns get replaced with a text label.
- `bind_rows()` then stacks that one-row summary underneath the main table.

> **Look at the label on the total row — does "Total Re-exports" correctly describe what this table is showing? What would you change it to?**
