...
# The file tradedata_ready.csv must be placed in the www/ folder.
# It is already cleaned and merged: country names, product labels, and
# flowCode2 ("Export" / "Import") are all included. No left_join needed!
#
# Expected columns:
...
tradedata <- read.csv("./www/tradedata_ready.csv")
...
The data has already been prepared for you: country names, product labels, and a readable trade-flow indicator (`flowCode2`) are all merged into a single file. You can focus entirely on the analysis!
...
### Have you completed the basic R tutorial?

```{r quiz0, echo = FALSE}
question("Have you completed the basic Use R! tutorial?  (*Check all that is TRUE*):",
...
We have a dataset of international merchandise trade transactions for **your country** (you will replace it with your own Comtrade file later).
...
The dataset has **`r nrow(tradedata)`** observations and **`r ncol(tradedata)`** variables.

> Use the black arrow at the top right of the table to navigate across columns if they don't all fit on screen.
...
## Exercise 1 — Explore with `select()`
...
## Exercise 2 — Filter exports with `filter()`
...
## Exercise 3 — Aggregate with `group_by()` and `summarise()`

A key task in trade analysis is computing **total trade values** by group — for example, total exports vs. total imports.

The combination of `group_by()` + `summarise()` is the tidyverse workhorse for this.
...
> **Trade balance:** is your country a net exporter or net importer overall?


## Exercise 4 — Trade by partner country
...
## Exercise 5 — Reshape with `pivot_wider()`
...
quiz(
  question("What does `pivot_wider()` do?",
    ...
    answer("It turns unique values of one column into new column names",
           correct = TRUE,
           message = "Exactly! Each unique value of `names_from` becomes a column."),
...
## Exercise 6 — Trade by product (2-digit HS)
...
## Exercise 7 — Visualise with `ggplot2`
...
```{r chart1, exercise = TRUE, exercise.eval = FALSE, exercise.setup = "chart-setup",
    exercise.cap = "Bar chart of trade flows by partner country"}
...
You have applied core `tidyverse` tools to **real trade statistics** from UN Comtrade:
...
These are the building blocks for the full analysis in the *L4 hands-on exercise*, where you will also handle raw data import, deduplication, and country/product label merging.

### Next steps

* **Apply this to your own country** — replace `tradedata_ready.csv` with your Comtrade export and run through the same steps.
* **Explore further** — try disaggregating by 6-digit HS code, or computing the **trade balance** (Export − Import) per partner.
* **Refer to the full L4 exercise** for the complete workflow including data loading and `left_join()`.
